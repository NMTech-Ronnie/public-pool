...
export class AppController {
  ...
  public async pool() {
    ...
    const userAgents = await this.clientService.getUserAgents();
  const totalHashRate = userAgents.reduce((acc, userAgent) => acc + Number(userAgent.totalHashRate ?? 0), 0);
  const totalMiners = userAgents.reduce((acc, userAgent) => acc + Number(userAgent.count ?? 0), 0);
