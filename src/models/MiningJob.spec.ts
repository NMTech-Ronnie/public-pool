...
function hexToAscii(hex: string): string {
  ...
  for (let i = 0; i < hex.length; i += 2) {
    ...
    if (char !== '\0') {
      // Ignore null characters to make sure string matching works
      ascii += char;
    }
  }
  return ascii;
...
describe('MiningJob', () => {
  ...
  describe('constructor', () => {
    beforeEach(() => {
      ...
      for (let i = 0; i < maxTransactions; i++) {
        block.transactions.push(
          bitcoinjs.Transaction.fromHex(
            '',
          ),
        );
      }
    ...
    it('should create a new MiningJob if POOL_IDENTIFIER is not set and use the default', () => {
      ...
      const miningIdentifier = extractPoolIdentifierFromScript(
        response.params[2],
      );
      ...
      configService.get = jest.fn((key: string) => {
        switch (key) {
          case 'POOL_IDENTIFIER':
            return expectedMiningIdentifier;
        }
        return null;
      });
    ...
    it('should remove pool identifier if script is too big with identifier', () => {
      ...
      configService.get = jest.fn((key: string) => {
        switch (key) {
          case 'POOL_IDENTIFIER':
            return 'A'.repeat(89); // 88 chars is the maximum size validated against bitcoin core in regtest
        }
        return null;
      });
      ...
      expect(console.warn).toBeCalledWith(
        'Pool identifier is too long, removing the pool identifier',
      );
      expect(miningIdentifier).toBe(expectedMiningIdentifier);
    });
  });
