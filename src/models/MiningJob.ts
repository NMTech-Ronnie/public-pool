...
export class MiningJob {
  ...
  /**
   * Fast path: compute only the 80-byte block header without copying any transactions.
   * Used for share difficulty validation (the hot path).
   * Only call copyAndUpdateBlock() for the extremely rare block-found case.
   */
  public computeHeaderBuffer(
    jobTemplate: IJobTemplate,
    versionMask: number,
    nonce: number,
    extraNonce: string,
    ...
    // Reconstruct serialized coinbase tx from pre-split parts + extraNonces
    // This avoids cloning the Transaction object entirely
    const serializedCoinbase =
      this.coinbasePart1 + extraNonce + extraNonce2 + this.coinbasePart2;
    const coinbaseBuffer = Buffer.from(serializedCoinbase, 'hex');
    const coinbaseHash = bitcoinjs.crypto.hash256(coinbaseBuffer);

    // Compute merkle root from new coinbase hash + cached merkle branches
    const merkleRoot = this.calculateMerkleRootHash(
      coinbaseHash,
      jobTemplate.merkle_branch,
    );
    ...
    // Build 80-byte block header directly
